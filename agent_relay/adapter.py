"""Agent Adapter v1 — unified dispatch abstraction for agent-relay.
...
from agent_relay.handoff import assign_current_task_agent, canonical_assignment_agent
from agent_relay.llm import review_task_service
...
def add_run_record(
    ...
    Useful for recording work done outside agent-relay (e.g. directly in
    ...
    if role not in ("writer", "reviewer"):
        raise AgentRelayError(f"Invalid role '{role}'. Must be 'writer' or 'reviewer'.")
...
class HumanInLoopAdapter(AgentAdapter):
    ...
    def dispatch(self, task_id, role="writer", **kwargs):
        ...
        if not worktree_path.exists():
            raise AgentRelayError(f"Worktree path does not exist: {worktree}")
...
class CliAgentAdapter(AgentAdapter):
    ...
    Uses ``run_cli_agent`` from ``agent_relay.executors`` to call the
    ...
    def dispatch(self, task_id, role="writer", timeout=120, **kwargs):
        ...
        result = run_cli_agent(
            ...
            command,
            prompt,
            cwd=agent_relay.config.PROJECT_ROOT,
            timeout=timeout,
        )
