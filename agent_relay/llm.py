...
def call_openai_compatible_chat(agent, prompt, system_prompt=None, dry_run=False):
    ...
    config = agent_relay.config.get_config()
    settings = agent_relay.config.get_llm_agent_settings(agent, config)
    payload = {
    ...
    if dry_run:
        print(
            json.dumps({k: v for k, v in settings.items() if k != "api_key"}, indent=4)
        )
        print("\n--- prompt ---")
        print(prompt)
        return None
...
def ask_agent_command(args):
    ...
    answer = call_openai_compatible_chat(
        ...
        args.prompt,
        system_prompt=(
            "You are a low-cost planning and review agent inside agent-relay. "
...
def get_task_diff_service(task, max_chars):
    ...
    if code != 0:
        ...
        code, diff, stderr = run_cmd(
            ["git", "diff", "HEAD", "--", "."], cwd=str(worktree_path)
        )
        if code != 0:
...
def review_task_service(
    task_id, agent="deepseek", max_diff_chars=10000, dry_run=False, mock=False
):
    import agent_relay.config
...
def review_task_command(args):
    ...
    try:
        config = agent_relay.config.get_config()
        settings = agent_relay.config.get_llm_agent_settings(args.agent, config)
        api_key_env = settings.get("api_key_env", "DEEPSEEK_API_KEY")

        if not os.environ.get(api_key_env) and not args.dry_run:
            print(
                f"Error: API Key environment variable '{api_key_env}' is not configured."
            )
            sys.exit(1)
