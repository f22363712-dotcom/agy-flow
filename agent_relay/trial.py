"""Value Trial Report — compare manual vs agent-relay experiment results.

Usage (via CLI): agent-relay value-report path/to/results.json
"""
...
def value_report(filepath):
    ...
    print("=" * 72)
    print(f"  agent-relay Value Trial Report: {trial_id}")
    print(f"  Task: {task_title}")
    print(f"  Date: {date}")
    print("=" * 72)
    print()
    print(f"{'Metric':<30} {'Manual':<12} {'agent-relay':<12} {'Delta':<12}")
    print("-" * 72)
    ...
    if a_time and b_time is not None and b_time <= a_time * 1.2:
        print(f"  ✅ Time not worse (Manual {a_time}min vs agent-relay {b_time}min)")
        passed += 1
    ...
    if passed >= 2:
        print(f"  ▶ Decision: ✅ Continue developing agent-relay")
    elif passed >= 1:
        print(f"  ▶ Decision: ⏸ Pause — value unclear; reduce friction first")
    else:
        print(f"  ▶ Decision: 🛑 Stop — agent-relay does not add value in current form")
