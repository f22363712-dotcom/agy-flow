...
VALID_STATES = frozenset(
    {
        ...
        "submitted",  # submitted via agent-relay submit
    }
)
...
def _get_state_file(task_id):
    return agent_relay.config.TASKS_DIR / f"{task_id}.state.json"
...
def set_task_state(task_id, state, reason=None, source_run_id=None):
    ...
    if state not in VALID_STATES:
        raise AgentRelayError(f"Invalid state '{state}'. Valid: {sorted(VALID_STATES)}")
