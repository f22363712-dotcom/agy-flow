...
def load_board():
    ...
    if not board_file.exists():
        print(
            f"Error: Board file not found at {board_file}. Run 'agent-relay init' first."
        )
        sys.exit(1)
...
def format_plan_summary(plan):
    ...
    for idx, step in enumerate(plan.get("recommended_pipeline", []), start=1):
        pipeline_lines.append(
            f"{idx}. {step.get('agent')} ({step.get('role')}): {step.get('purpose')}"
        )
...
def estimate_and_log_cost(
    task_id, agent, manual_input=0, manual_output=0, input_prompt=""
):
    ...
    costs = load_costs()
    task_costs = costs["tasks"].get(
        task_id, {"input_tokens": 0, "output_tokens": 0, "cost": 0.0}
    )
    ...
    for i, line in enumerate(lines):
        if "|" in line:
            ...
            if parts and parts[0] == task_id:
                ...
                new_worktree = (
                    worktree if worktree else (parts[5] if len(parts) > 5 else "")
                )
                lines[i] = f"| {task_id} | {parts[1]} | {parts[2]} | {status} | {
                    new_branch
                } | {new_worktree} |\n"
                updated = True
                break
...
def check_inside_project(command):
    """Verify that we are executing from within an initialized agent-relay repository."""
    if not CONFIG_FILE.exists():
        print(
            f"Error: Not inside an initialized agent-relay repository. CONFIG_FILE: {CONFIG_FILE}"
        )
        print("Please run 'agent-relay init' in the project directory first.")
        sys.exit(1)


def init_project(args):
    """Initializes the agent-relay structure in the current working directory."""
    global \
        PROJECT_ROOT, \
        AGENTS_DIR, \
        TASKS_DIR, \
    ...
    print(f"Initializing agent-relay collaboration framework in {PROJECT_ROOT}...")
    ...
    agent_relay.config.PROJECT_ROOT = PROJECT_ROOT
    agent_relay.config.AGENTS_DIR = AGENTS_DIR
    agent_relay.config.TASKS_DIR = TASKS_DIR
    agent_relay.config.TEMPLATES_DIR = TEMPLATES_DIR
    agent_relay.config.LOGS_DIR = LOGS_DIR
    agent_relay.config.CONFIG_FILE = CONFIG_FILE
    agent_relay.config.BOARD_FILE = BOARD_FILE
    agent_relay.config.COSTS_FILE = COSTS_FILE
...
def create_task(args):
    ...
    workspace_path = ""
    if workspace_name:
        from agent_relay.workspaces import get_workspace

        ws = get_workspace(workspace_name)
        workspace_path = ws["path"]
    else:
        from agent_relay.workspaces import resolve_workspace

        resolved_name, resolved_path = resolve_workspace()
        workspace_name = resolved_name
        workspace_path = resolved_path or ""

    if args.agent is None:
        pipeline = route_plan.get("recommended_pipeline", [])
        if pipeline and isinstance(pipeline[0], dict):
            agent = "->".join(step.get("agent", "claude") for step in pipeline)
        else:
            agent = "->".join(pipeline) if pipeline else "claude"
    else:
        agent = args.agent
    ...
    if workspace_name:
        task_content += f"- **Workspace**: {workspace_name} ({workspace_path})\n"
        route_plan["workspace_name"] = workspace_name
        route_plan["workspace_path"] = workspace_path
    task_content += "\n"
    ...
    plan_file.write_text(
        json.dumps(route_plan, indent=4, ensure_ascii=False), encoding="utf-8"
    )
    ...
    code, stdout, stderr = run_cmd(
        [
            ...
            str(plan_file.relative_to(PROJECT_ROOT)),
            str(BOARD_FILE.relative_to(PROJECT_ROOT)),
        ]
    )
    if code != 0:
        raise AgentRelayError(f"Git add failed in create_task: {stderr}")

    code, stdout, stderr = run_cmd(
        ["git", "commit", "-m", f"docs(task): create {task_id} - {title}"]
    )
    if code != 0:
        raise AgentRelayError(f"Git commit failed in create_task: {stderr}")

    print(f"\nTask {task_id} successfully created and committed in Git!")
    print(f"To start working on this task, run: agent-relay start {task_id}\n")
...
def start_task(args):
    ...
    if not task:
        raise AgentRelayError(f"Task '{task_id}' not found.")

    if task["status"] != "Todo":
        raise AgentRelayError(f"Task '{task_id}' is already in status '{task['status']}'")
    ...
    if code != 0:
        raise AgentRelayError(f"Failed to create git worktree: {stderr}")
...
"""
    ...
    task_metadata = {
        ...
        "title": task["title"],
        "agent": agent,
        "writer": canonical_assignment_agent(agent),
        "reviewers": [],
        "mode": "handoff",
        "role": "writer",
        "status": "In Progress",
    ...
    try:
        ...
        print(
            f"Created task metadata file for routing enforcement: {current_task_path}"
        )
    except Exception as e:
        print(f"Warning: Failed to create current_task.json: {e}")

    code_add, _, err_add = run_cmd(
        ["git", "add", str(BOARD_FILE.relative_to(PROJECT_ROOT))]
    )
    if code_add != 0:
        raise AgentRelayError(f"Git add failed in start_task: {err_add}")
    code_commit, _, err_commit = run_cmd(
        ["git", "commit", "-m", f"chore(task): start {task_id} - setup worktree"]
    )
    if code_commit != 0:
        raise AgentRelayError(f"Git commit failed in start_task: {err_commit}")

    if (
        agent == "codex"
        and getattr(args, "launch_editor", True)
        and os.environ.get("AGY_FLOW_TESTING") != "1"
    ):
...
def submit_task(args):
    ...
    if "In Progress" not in task["status"]:
        raise AgentRelayError(
            f"Task '{task_id}' is not in progress. Current status: '{task['status']}'"
        )

    worktree_path = Path(task["worktree"])
    if not worktree_path.exists():
        raise AgentRelayError(f"Worktree path does not exist: {worktree_path}")
    ...
    if test_cmd:
        ...
        res = subprocess.run(
            test_args, cwd=str(worktree_path), capture_output=True, text=True
        )
        print("Test Output:\n" + res.stdout)
        if res.returncode != 0:
            print(res.stderr)
            raise AgentRelayError(
                f"Validation tests failed (exit code: {
                    res.returncode
                }). Submit aborted."
            )
    ...
    if next_step:
        ...
        print(
            f"Handoff target detected: transitioning from {active_agent} to {next_agent}..."
        )

        # Carry forward existing reviewers during handoff
        existing_meta = {}
        if current_task_path.exists():
            try:
                ...
                pass
        existing_reviewers = existing_meta.get("reviewers", [])
        assign_current_task_agent(next_agent, task_id, reviewers=existing_reviewers)

        next_guide = worktree_path / config["agents"].get(next_agent, {}).get(
            "guide_file", "CLAUDE.md"
        )
        if next_guide.name != "CLAUDE.md" or next_agent == "claude":
            routing_plan_text = format_plan_summary(
                {"recommended_pipeline": handoff_steps, "task_type": "handover"}
            )
...
"""
        ...
        if code_wadd != 0:
            raise AgentRelayError(f"Git add failed in worktree handover: {err_wadd}")
        code_wcommit, _, err_wcommit = run_cmd(
            [
                "git",
                "commit",
                "-m",
                f"chore(task): handover task-{task_id} from {active_agent} to {next_agent}",
            ],
            cwd=str(worktree_path),
        )
        if code_wcommit != 0:
            raise AgentRelayError(f"Git commit failed in worktree handover: {err_wcommit}")

        next_status = f"In Progress ({next_agent})"
        update_board_row(task_id, next_status)
        code_badd, _, err_badd = run_cmd(
            ["git", "add", str(BOARD_FILE.relative_to(PROJECT_ROOT))]
        )
        if code_badd != 0:
            raise AgentRelayError(f"Git add board failed in handover: {err_badd}")
        code_bcommit, _, err_bcommit = run_cmd(
            [
                "git",
                "commit",
                "-m",
                f"chore(task): transition {task_id} status to {next_agent}",
            ]
        )
        if code_bcommit != 0:
            raise AgentRelayError(f"Git commit board failed in handover: {err_bcommit}")
    ...
    code_wadd, _, err_wadd = run_cmd(["git", "add", "."], cwd=str(worktree_path))
    if code_wadd != 0:
        raise AgentRelayError(f"Git add failed in worktree submit: {err_wadd}")
    ...
    if not stdout:
        ...
        if code != 0:
            raise AgentRelayError(f"Git commit failed in worktree submit: {stderr}")
    ...
    if code_badd != 0:
        raise AgentRelayError(f"Git add board failed in submit: {err_badd}")
    code_bcommit, _, err_bcommit = run_cmd(
        ["git", "commit", "-m", f"chore(task): submit {task_id} for review"]
    )
    if code_bcommit != 0:
        raise AgentRelayError(f"Git commit board failed in submit: {err_bcommit}")
...
def merge_task(args):
    ...
    if code != 0:
        raise AgentRelayError(
            f"Merge conflict or failure: {stderr}. Please resolve manually."
        )
    ...
    if code_del != 0:
        raise AgentRelayError(f"Failed to delete branch {branch_name}: {err_del}")
    ...
    if code_badd != 0:
        raise AgentRelayError(f"Git add board failed in merge: {err_badd}")
    code_bcommit, _, err_bcommit = run_cmd(
        ["git", "commit", "-m", f"chore(task): complete and merge {task_id}"]
    )
    if code_bcommit != 0:
        raise AgentRelayError(f"Git commit board failed in merge: {err_bcommit}")
...
def status_tasks(args):
    ...
    for t in tasks:
        print(
            f"{t['id']:<10} | {t['title'][:40]:<40} | {t['agent']:<20} | {t['status']:<15}"
        )
    ...
    print(
        f"  Remaining Pool          : ${max(0.0, total_budget - accumulated):.5f} USD"
    )
    print("-" * 100)
    if task_breakdown:
...
def update_module_paths():
    ...
    TEMPLATES_DIR = agent_relay.config.TEMPLATES_DIR
    LOGS_DIR = agent_relay.config.LOGS_DIR
    CONFIG_FILE = agent_relay.config.CONFIG_FILE
    BOARD_FILE = agent_relay.config.BOARD_FILE
    COSTS_FILE = agent_relay.config.COSTS_FILE
