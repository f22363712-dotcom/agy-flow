...
exported to the format expected by ``agent-relay value-report``.
"""
...
def _trials_dir():
    d = agent_relay.config.AGENTS_DIR / "trials"
...
def _load(trial_id):
    path = _path(trial_id)
    if not path.exists():
        raise AgentRelayError(f"Trial '{trial_id}' not found at {path}.")
    try:
        return json.loads(path.read_text(encoding="utf-8"))
    except Exception as e:
        raise AgentRelayError(f"Failed to read trial '{trial_id}': {e}")


def trial_start(trial_id, task_title, track="agent-relay"):
    ...
    task_title : str
    track : str
        ``"manual"`` or ``"agent-relay"`` — maps to ``track_a`` / ``track_b``.
    ...
    path = _path(trial_id)
    if path.exists():
        raise AgentRelayError(f"Trial '{trial_id}' already exists at {path}.")
...
def trial_export(trial_id, output_path=None):
    ...
    ``agent-relay value-report`` can read it.
    ...
    export = {
        ...
        track_key: counters,
        "_source": "agent_relay_trial_recorder",
    }
