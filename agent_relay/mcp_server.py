"""MCP Server v2 — expose agent-relay core capabilities as MCP tools + resources.
...
Runs over stdin/stdout (JSON-RPC 2.0).  Start with ``agent-relay mcp`` or
directly: ``python -m agent_relay.mcp_server``.
"""
...
def _require(args, key, label=None):
    """Require *key* in *args*, raising AgentRelayError if missing."""
    if key not in args:
...
def _handle_call(name, args):
    ...
    from agent_relay.router import route_task_by_id, route_task
    ...
    from agent_relay.doctor import task_status, doctor
    ...
    if name == "agy_route_task":
        ...
        if task_id:
            ...
            raise AgentRelayError("Provide task_id or title to route.")
        return {
            "content": [
                {
                    "type": "text",
    ...
    raise AgentRelayError(f"Unknown tool: {name}")
...
def _handle_read_resource(uri: str) -> dict:
    ...
    raise AgentRelayError(f"Unknown resource URI: {uri}")
    ...
    if method == "initialize":
        return {
            ...
            "result": {
                ...
                "serverInfo": {"name": "agent-relay-mcp", "version": "2.0.0"},
            },
        }
...
def main():
    """Entry point for ``agent-relay mcp``."""
    run_mcp_server()
