# 多 Coding Agent 协同工作框架 (agent-relay) 使用手册 (v2)

本手册详细介绍了 `agent-relay` 全局协同框架的设计理念、配置规范、命令行使用以及针对 **Claude Code**、**Antigravity** 和 **Codex** 三个不同 Agent 的具体协同开发指南。

---

## 一、 全局架构与设计理念

`agent-relay` 采用 **“项目根路径向上检索”** 机制（类似于 Git 工作原理），支持您在项目内的任意子目录直接调用全局指令，使其成为您整个开发流程中无感知的“开发基座”：

*   **全局命令行**：支持在任意终端直接输入 `agent-relay <command>` 启动，无需切换到特定安装目录。
*   **自适应项目查找**：当您在项目的任何子目录（如 `D:\my-project\src\components`）执行 `agent-relay status` 时，工具会自动向上遍历父级目录寻找 `.agents` 文件夹，锁定当前激活的项目根目录。
...
系统已经为您在 `D:\tools` 目录下创建了 `agent-relay.bat` 包装脚本，并将 `D:\tools` 成功添加至您的用户环境变量 `Path` 中。
...
- `agent_registry`: 路由层配置，描述 Agent 的 `kind`、入口、能力、成本模式。`agent-relay plan` 和 `agent-relay create` 会优先读取这里生成结构化计划。
...
### 1. 初始化新项目
在您的任意项目根目录下（如您的新开发文件夹内）执行，将该目录初始化为 `agent-relay` 项目（自动创建 `.agents/` 看板与配置并执行 `git init`）：
```bash
agent-relay init
```
...
```bash
agent-relay create "任务标题" --agent <claude|antigravity|codex>
```
...
```bash
agent-relay plan "编写后端登录验证接口，设计前端 HTML 登录框并进行页面视觉走查"
```
...
```bash
agent-relay assign codex
agent-relay assign antigravity
```

可选记录当前任务：

```bash
agent-relay assign antigravity --task-id task-010
```

v4 起，守卫文件采用“写入者 + 审查者”协议，同时保留旧版 `agent` 字段兼容现有 Agent 守卫：
...
### 6. 启动任务并拉起隔离区
```bash
agent-relay start <task-id>
```

### 7. 在隔离区完成开发后提审
```bash
agent-relay submit <task-id> [--test-cmd "测试指令"]
```
...
```bash
agent-relay handoff-plan task-009
```

### 8. 合并代码并销毁隔离区
```bash
agent-relay merge <task-id>
```
...
agent-relay ask deepseek "把这个需求拆成验收标准"
agent-relay review task-010 --agent deepseek
```

验证配置但不实际调用 API：

```powershell
agent-relay ask deepseek "hello" --dry-run
agent-relay review task-010 --agent deepseek --dry-run
```
