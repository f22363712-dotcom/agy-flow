...
## 任务背景
在 `D:\multi_agent_collaboration` 中继续开发 `agent-relay` 协同框架。本项目旨在构建一个本地协同框架，将任务根据能力和成本高效路由给不同的 Agent（如 Codex、Antigravity、DeepSeek 等），实现便宜优先（cheap-first）、可审计、可接力的跨平台协同。
...
- 实现 `AgentRelayHTTPHandler(BaseHTTPRequestHandler)`，定义以下端点：
  - **读端点**：
...
### 3. 自动化集成测试脚本
修改文件：`test_serve.py`
- 使用 Python 动态模块加载技术载入带有连字符的 `agent-relay.py`。
