...
from agy_flow_classify import DEFAULT_AGENT_REGISTRY
...
def update_paths(new_root=None):
    ...
    try:
        ...
        agy_flow.tasks.update_module_paths()
    except (ImportError, AttributeError):
        pass
    try:
        import agy_flow.handoff
        agy_flow.handoff.update_module_paths()
    except (ImportError, AttributeError):
        pass
    try:
        import agy_flow.llm
        agy_flow.llm.update_module_paths()
    except (ImportError, AttributeError):
        pass
...
def get_llm_agent_settings(agent, config=None):
    ...
    if registry_config.get("kind") != "llm_api" and runtime_config.get("provider") not in {
        "litellm",
        "openai_compatible",
    }:
