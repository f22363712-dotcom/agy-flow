...
def _registry_path():
    return agy_flow.config.AGENTS_DIR / "workspaces.json"
...
def get_workspace(name):
    ...
    if ws is None:
        raise AgyFlowError(
            f"Workspace '{name}' not found. Use 'agy-flow workspace list'."
        )
    return {
        "name": name,
        "path": ws.get("path", ""),
...
def add_workspace(name, path, description="", default_tests=None):
    ...
    if not name or " " in name:
        raise AgyFlowError("Workspace name must be non-empty and contain no spaces.")
...
def remove_workspace(name):
    ...
    if name not in workspaces:
        raise AgyFlowError(f"Workspace '{name}' not found.")
    del workspaces[name]
